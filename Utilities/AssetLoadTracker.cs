...
#if DEBUG
                SearchableBuildMenuPlugin.SearchableBuildMenuLogger.LogError($"Mapped original name {prefab} [{simpleName}] to {bundle.name}");
#endif
            }
        }
    }
    ...
    internal static void MapBundlesToAssemblies()
    {
        ...
        foreach (string? bundleName in PrefabToBundleMapping.Values.Distinct())
        {
            foreach (Assembly? assembly in allAssemblies)
            {
                ...
                {
                    SearchableBuildMenuPlugin.SearchableBuildMenuLogger.LogError($"Error while getting manifest resource names for assembly {assembly.GetName().Name}: {e}");
                }
            }
        }
    }
